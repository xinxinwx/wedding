 ...
 */
  foo: function () {
    if (that.data.inputValue.indexOf('习近平')>=0){
      wx.showToast({
        title: '抱歉，不能包含敏感字符',
        icon: "none"
      })
  ...
  */
  getPageData: function () {
    ...
    wx.cloud.callFunction({
      name: "msglistcount",
      success(res) {
        console.log("数量", res.result.total)
    ...
    var size = that.data.pagersize
      // .skip(ye).limit(size).orderBy('msgtime', 'desc')
    console.log("猜测", ye)
    wx.cloud.callFunction({
      name: "friendMsgList",
      data: {
        size: that.data.pagersize,
        ye: that.data.pagersize * that.data.pagernumber
      },
      success(res) {
